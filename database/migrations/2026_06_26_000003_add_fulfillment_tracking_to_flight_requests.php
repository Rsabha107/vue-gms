...
return new class extends Migration {
    public function up(): void
    {
        Schema::table('flight_requests', function (Blueprint $table) {
            $table->unsignedBigInteger('fulfilled_by_id')->nullable()->after('note');
            $table->unsignedBigInteger('fulfills_request_id')->nullable()->after('fulfilled_by_id');

            $table->foreign('fulfilled_by_id')->references('id')->on('flight_requests')->nullOnDelete();
            $table->foreign('fulfills_request_id')->references('id')->on('flight_requests')->nullOnDelete();
        });
    }
